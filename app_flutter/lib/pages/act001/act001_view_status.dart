...
class _ActivityStatusDetailViewState extends State<ActivityStatusDetailView>
  ...
  Future<List<_StatusRow>> _fetchStatusRows(String type) async {
    final range = _selectedDateRange;
    final rows = await ActivityApiService().getStatusRows(
      type: type,
    ...
    return type == 'by-store'
        ? _mapStoreStatusRows(rows, range.$1, range.$2)
        : _mapAssigneeStatusRows(rows, range.$1, range.$2);
  }
  ...
  List<_StatusRow> _mapStoreStatusRows(
    ...
    for (final row in rows) {
      final storeNm = row.jsonString('storeNm');
    ...
    for (final row in rows) {
      ...
      final count = row['count'].asJsonInt();
  ...
  List<_StatusRow> _mapAssigneeStatusRows(
    ...
    // 모든 수퍼바이저를 먼저 추가
    for (final row in rows) {
      final userName = row.jsonString('userName');
