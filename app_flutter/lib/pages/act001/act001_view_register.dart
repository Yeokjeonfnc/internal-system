...
import 'package:app_flutter/pages/act001/act001_dialog_approval_line.dart';
import 'package:app_flutter/core/utils/json_extensions.dart';
import 'package:app_flutter/pages/act001/act001_api_service.dart';
import 'package:app_flutter/pages/act001/act001_dialog_instructions.dart';
import 'package:app_flutter/pages/act001/act001_dialog_visit_history.dart';
import 'package:app_flutter/pages/act001/act001_api_checklist.dart';
...
import 'package:app_flutter/pages/str001/str001_api.dart';
...
class _ActivityRegisterViewState extends State<ActivityRegisterView> {
  ...
  Future<void> _loadActivity(int actIdx) async {
    ...
    Store? store;
    final rawStoreIdx = asJsonIntOpt(activity['storeIdx']);
    if (rawStoreIdx != null) {
    ...
    final svIdFromApi = activity.jsonString('svId');

    setState(() {
      _selectedStore = store;
      final actType = activity.jsonString('actType');
      _activityKind = actType.isNotEmpty ? actType : _activityKind;
      _activityDate = _parseDate(activity['actDt']) ?? _activityDate;
      _specialNotesController.text = activity.jsonString('memoTxt');
      _activityNotesController.text = activity.jsonString('actNotes');
      _suggestionsController.text = activity.jsonString('suggestions');
      _svNotesController.text = activity.jsonString('svNotes');
      final appr = activity.jsonString('apprStatus').trim();
      ...
      if (svIdFromApi.isNotEmpty) {
        _deptNm = activity.jsonString('svDeptNm');
        _drafterNm = activity.jsonString('svNm');
      }
    });
  ...
  Future<void> _restoreApprovalLineFromActivity(
    ...
    if (fromApi is List) {
      ...
      final raw = activity.jsonString('apprId');
      if (raw.isNotEmpty) {
        ids.addAll(
    ...
    final svId = activity.jsonString('svId');
    final authProvider = context.read<AuthProvider>();
    final authId = authProvider.userId.trim();

    try {
      final users = await Emp001ApiService().getUsers();
  ...
  Future<void> _autoSaveDraft() async {
    ...
    _autoSaving = false;
    if (saved != null) {
      final rawActIdx = asJsonIntOpt(saved['actIdx']);
      if (rawActIdx != null) {
...
class _ChecklistBlockState extends State<_ChecklistBlock> {
  ...
  Future<void> _loadChecklists() async {
    ...
    try {
      final items = await Act001ChecklistApi().getChecklistsByBrand(
  ...
  void _syncResultFromPending() {
    ...
    for (final row in pending) {
      final chkIdx = asJsonIntOpt(row['chkIdx']);
