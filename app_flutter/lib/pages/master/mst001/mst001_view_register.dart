...
/// 사원 등록 화면.
///
/// 폼 필드는 [User] 의 의미(이름·부서명·직급명·휴대전화·이메일·태그)와 맞추고,
/// 저장 시 [User.buildCreateUserRequest] 로 `/users` POST 본문을 만든다.
...
class _UserRegisterViewState extends ConsumerState<UserRegisterView> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final body = User.buildCreateUserRequest(
        name: _nameCtrl.text,
  ...
  Widget build(BuildContext context) {
    ...
    return DetailScreenWithTabs(
      ...
      tabTitles: UserRegisterView._tabTitles,
      tabPages: [
