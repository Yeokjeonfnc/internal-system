...
import 'package:app_flutter/pages/development/dev001/dev001_api.dart';
...
class ApiPartnerRepository implements PartnerRepository {
  ...
  @override
  Future<List<Partner>> all() => _apiService.fetchList();

  @override
  Future<Partner?> find(int partnerIdx) => _apiService.fetchOne(partnerIdx);
...
class PartnerNotifier extends BaseListNotifier<PartnerFilter, Partner> {
  ...
  @override
  List<ListFilterRule<PartnerFilter, Partner>> get ruleList {
    final regionOpts =
        ref.watch(partnerCodeOptionsProvider(20)).value ?? const <CodeOption>[];
    return dev001ListRules(regionOpts);
  }

  void setKeyword(String v) => state = state.copy(partnerKeyword: v);
  void setStatus(String v) => state = state.copy(partnerStatus: v);
