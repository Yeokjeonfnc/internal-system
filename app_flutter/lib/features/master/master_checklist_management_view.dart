...
import 'package:app_flutter/features/master/master_checklist_api_service.dart';
    ...
    return FutureBuilder<List<MasterChecklistItem>>(
      ...
      builder: (context, snapshot) {
        ...
        return _ChecklistListShell(
          ...
          countText: '총 ${rows.length}건 · 총배점 $totalScore점',
          onRefresh: _refresh,
...
class _ChecklistCreateDialogState extends State<_ChecklistCreateDialog> {
  ...
  Future<void> _save() async {
    ...
    final payload = {
      ...
      'chkType': _chkType,
      'chkContent': content,
      'baseScore': int.tryParse(_scoreCtrl.text.trim()) ?? 0,
      'useYn': _required ? 'Y' : 'N',
    };
    ...
    final saved = item == null
        ...
        : await api.updateChecklist(item.chkIdx, payload);
    if (!mounted) return;
